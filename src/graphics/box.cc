...
void drawBox(const schema::Data &data, const schema::BoxPos &box_pos,
             const schema::BoxType &box_type, const Color &color) {
...
void drawBoxOutline(const schema::Data &data, const schema::BoxPos &box_pos,
  ...
  DrawCubeWires(
      ...
      makeColor((255u - color.r), (255u - color.g), (255u - color.b), 255u));
}

Vector3 getSize(const schema::BoxPos &box_pos,
                const schema::BoxType &box_type) {
  const auto &rotated = box_pos.rotated();
  return makeVector3(!rotated ? box_type.size_x() : box_type.size_y(),
                     box_type.size_z(),
                     !rotated ? box_type.size_y() : box_type.size_x());
}

Vector3 getPosition(const schema::BoxPos &box_pos) {
  return makeVector3(box_pos.x(), box_pos.z(), box_pos.y());
}

Vector3 getItemSize(const schema::BoxPos &box_pos,
                    const schema::BoxType::Item &item, const schema::Sku &sku) {
  const auto &rotated = item.rotated;
  auto item_size =
      makeVector3(!rotated ? sku.size_x() : sku.size_y(), sku.size_z(),
...
Vector3 getItemPosition(const schema::BoxPos &box_pos,
